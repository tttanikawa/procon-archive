...
fn run() {
    ...
    let cumsum = std::iter::once(&('-', 0))
        ...
        .collect_vec();
    let mut ans = 0_usize;
    for start in 0..comp.len() {
        let end = if comp[start].0 == '0' {
            cmp::min(start + 2 * (k - 1) + 1, cumsum.len() - 2)
        } else {
            cmp::min(start + 2 * k, cumsum.len() - 2)
        };
        ans = cmp::max(ans, cumsum[end + 1] - cumsum[start]);
    }
    println!("{}", ans);
}
