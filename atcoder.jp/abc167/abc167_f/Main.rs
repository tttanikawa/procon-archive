...
#[test]
fn check_test() {
    assert_eq!(check(&"()".chars().collect::<Vec<char>>()), (0, 0));
    assert_eq!(check(&")(".chars().collect::<Vec<char>>()), (1, 1));
    assert_eq!(check(&")((())".chars().collect::<Vec<char>>()), (1, 1));
    assert_eq!(check(&"()))(((".chars().collect::<Vec<char>>()), (2, 3));
}
...
fn run() {
    ...
    for _ in 0..n {
        let s = sc.read_chars();
        let (min_v, total) = check(&s);
        if total >= 0 {
            pos.push((-min_v, total));
        } else {
            neg.push((-(min_v - total), -total));
        }
    }

    pos.sort_by_key(|&(min_v, total)| min_v);
    neg.sort_by_key(|&(min_v, total)| min_v);
    // neg.reverse();

    let mut left_stacked = 0;
    for (min_v, total) in pos.into_iter() {
        if min_v > left_stacked {
            println!("No");
            return;
        }
        left_stacked += total;
    ...
    let mut right_stacked = 0;
    for (min_v, total) in neg.into_iter() {
        if min_v > right_stacked {
            println!("No");
            return;
        }
        right_stacked += total.abs();
    }

    let ans = left_stacked == right_stacked;
    yn(ans);
}
