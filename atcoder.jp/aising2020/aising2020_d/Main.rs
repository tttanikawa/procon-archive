...
fn run() {
    ...
    let n: usize = sc.read();
    let x: Vec<char> = sc.read_chars();

    let pcount = x.iter().filter(|&&x| x == '1').count();
    if pcount == 0 {
        println!("{}", n);
        return;
    }
    let mut base_plus = ModInt::new(0, pcount + 1);
    let mut tmp = ModInt::new(2, pcount + 1);
    for i in 0..n {
        if x[i] == '1' {
            base_plus += tmp.pow(n - i - 1);
        }
    }
    let mut base_minus = ModInt::new(0, pcount);
    if pcount > 1 {
        base_minus = ModInt::new(0, pcount - 1);
        let mut tmp = ModInt::new(2, pcount - 1);
        for i in 0..n {
            if x[i] == '1' {
                base_minus += tmp.pow(n - i - 1);
            }
        }
    }

    for i in 0..n {
        let pcount_i = if x[i] == '0' { pcount + 1 } else { pcount - 1 };
        if pcount_i > 0 {
            let sub = ModInt::new(2, pcount_i);
            let base_i = if x[i] == '0' {
                base_plus + sub.pow(n - i - 1)
            } else {
                base_minus + pcount_i - sub.pow(n - i - 1)
            };

            let mut base_i: usize = base_i.value();
            let mut ans = 1_usize;
            while base_i > 0 {
                base_i %= base_i.count_ones() as usize;
                ans += 1;
            }
            println!("{}", ans);
        } else {
