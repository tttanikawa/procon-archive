...
fn dfs(n: usize, memo: &mut Vec<Option<ModInt<usize>>>) -> ModInt<usize> {
    if let Some(v) = memo[n] {
        return v;
    }

    if n < 3 {
        return ModInt::new(0, 1e9 as usize + 7);
    }

    let mut ans = ModInt::new(1, 1e9 as usize + 7);
    for i in 3..=n {
        ans += dfs(n - i, memo);
    }
    memo[n] = Some(ans);
...
fn run() {
    ...
    let mut memo = vec![None; s + 1];
    let ans = dfs(s, &mut memo);
