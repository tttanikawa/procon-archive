...
fn run() {
    ...
    let n: usize = sc.read();
    let ab = sc.read_pairs::<usize>(n);

    let mut a = ab.iter().map(|x| x.0).collect::<Vec<usize>>();
    let mut b = ab.iter().map(|x| x.1).collect::<Vec<usize>>();

    a.sort();
    b.sort();

    if n % 2 == 0 {
        let min = a[n / 2 - 1] + a[n / 2];
        let max = b[n / 2 - 1] + b[n / 2];
        let ans = max - min + 1;
        println!("{}", ans);
    } else {
        let min = a[n / 2];
