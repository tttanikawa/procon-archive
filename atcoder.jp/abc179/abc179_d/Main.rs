...
fn main() {
    ...
    let n: usize = sc.read();
    let k: usize = sc.read();
    let lr = sc.read_pairs::<usize>(k);
    const MOD: i64 = 998244353;

    let mut ans = vec![ModInt::new(0, MOD); 2 * n + 1];
    ans[1] = ModInt::new(1, MOD);
    for start in 1..n {
        ans[start] = ans[start - 1] + ans[start];
        for &(l, r) in lr.iter() {
            ans[start + l] = ans[start + l] + ans[start];
            ans[start + r + 1] = ans[start + r + 1] - ans[start];
        }
    }
